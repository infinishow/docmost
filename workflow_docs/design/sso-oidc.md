...
기존 `auth_providers`/`auth_accounts` DB 스키마와 프론트엔드 `SsoLogin` 컴포넌트를 최대한 활용한다. 서버 시작 시 환경변수에서 Google provider를 DB에 seed하고, 새로운 `GoogleOAuthModule`에서 Passport 전략 + 콜백 컨트롤러를 구현한다. 프론트엔드는 기존 EE `SsoLogin` 컴포넌트가 `authProviders` 배열을 자동 렌더링하므로 추가 작업 불필요.
...
| 3 | `apps/server/src/core/auth/google-oauth/google-oauth.controller.ts` | GET `/api/sso/google/login` (인증 시작), GET `/api/sso/google/callback` (콜백 처리) | `auth.controller.ts:AuthController` | R-1, R-2, R-3 |
| 4 | `apps/server/src/core/auth/google-oauth/google-oauth.service.ts` | 사용자 조회/생성, 계정 연결, 도메인 검증 로직 | `services/auth.service.ts:AuthService` | R-4, R-5, R-3 |
| 5 | `apps/server/src/core/auth/google-oauth/google-oauth-seed.service.ts` | 서버 시작 시 환경변수 → auth_providers seed/sync | `services/signup.service.ts:SignupService` (DB insert 패턴) | R-3, R-6 |
...
| 8 | `apps/server/src/app.module.ts` | GoogleOAuthModule 조건부 import (환경변수 존재 시만) | 기존 module imports | R-6 |
...
- **Test intent**: 환경변수에서 Google OAuth 설정을 읽고, 서버 시작 시 auth_providers 테이블에 Google provider가 생성/동기화되는지 검증
...
- **Test intent**: Google 콜백 후 신규 사용자 생성, 기존 사용자 연결, JWT 발급 및 리다이렉트가 정상 동작하는지 검증
...
- **Test intent**: 허용되지 않은 도메인 거부, 동의 거부 시 에러 리다이렉트, 환경변수 미설정 시 모듈 미로딩 검증
- **Changed files**: `google-oauth.service.ts`, `app.module.ts`
